...
public class LogSaveService extends Service {
	...
	public void onCreate() {
	    ...
	    //将该通知显示为默认View
	    Time time = new Time();
	    String pathString = mPrefs.getString(getString(R.string.pref_log_path), "/mnt/sdcard");
	    DateFormat df5 = new SimpleDateFormat("yyyy-MM-dd-hh-mm", Locale.US); 
	    
	    mSaveFile = pathString+"/log_"+df5.format(new Date())+".txt";
	    mNotification.setLatestEventInfo(this, "Log Saving", "Save to "+mSaveFile,null);
	...
	class LogReaderAsyncTask extends AsyncTask<Integer, LogLine, Boolean> {
		...
		protected Boolean doInBackground(Integer... params) {
			   try {
		        	...
		        	Toast.makeText(LogSaveService.this, "保存出错了！！", Toast.LENGTH_SHORT).show();
		            e.printStackTrace();
		            ok = false;
